    ...
    def read(self, request, account=None, phone=None, si=None, tariff=None):
        ...
        if user.has_perm("billing.api_view"):
            if phone is not None and si is not None and tariff is not None:
                ...
                return {"lcr_rate": resp.rate, "suffix": resp.suffix, "lcr_digits": resp.digits, "lcr_carrier": resp.gw, "lcr_price": resp.price, "lcr_currency": resp.currency, "lcr_name": resp.name, "nibble_rate": "0.56" }
            else:
                return rc.NOT_HERE
        else:
            return rc.FORBIDDEN
